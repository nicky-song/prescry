...
export function buildPatientCommunication(languageCodePreference) {
  const languagePreference = languageMap(languageCodePreference);
    ...
    return [
      {
        language: {
          coding: [
            {
              code: languageCodePreference,
              system: LANGUAGE_SYSTEM,
            },
          ],
          text: languagePreference,
        },
        preferred: true,
      },
    ];
...
export function getPatientId(patientProfile) {
  const [, id] = patientProfile.split("/");
  return id;
}
