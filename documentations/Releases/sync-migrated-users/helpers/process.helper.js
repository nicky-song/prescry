...
export async function batchAndLogStep(scope, processor, noLimit = false) {
  ...
  while (!isFinished) {
    ...
    if (
      ...
      !getterResponse.skippedUsers?.length
    ) {
    ...
    logProgress(
      `${scope}: Batch result - successful: ${
        getterResponse.successfulUsers.length
      }, failures: ${getterResponse.exceptionUsers.length}${
        getterResponse.skippedUsers
          ? `, skipped: ${getterResponse.skippedUsers?.length}`
          : ''
      }`
    );
    logTime();
