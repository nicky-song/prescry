...
import { planSingleTestData, pbmUser } from '../../test-data';
...
test('Keep current prescription @exec', async ({
  ...
  await test.step(
    `Generate Claim Link "${planSingleTestData.planLinkScenario}" and Navigate to it`,
    async () => {
      const link = await generateClaimLink(
        person,
        planSingleTestData.planLinkScenario
      );
  ...
  await test.step('Assert alternative medication savings', async () => {
    await claimAlertsMedicationTagsValidator(planSingleTestData.tagsStatus);
  });
  ...
  await test.step('Keep current prescription with a link', async () => {
    ...
    await expect(page).toHaveURL(planSingleTestData.regExpCabinet);
  });
});
