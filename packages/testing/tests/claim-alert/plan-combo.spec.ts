...
import { planComboTestData, pbmUser } from '../../test-data';
...
test('Keep current prescription @exec', async ({
  ...
  await test.step(
    `Generate Claim Link "${planComboTestData.planLinkScenario}" and Navigate to it`,
    async () => {
      const link = await generateClaimLink(
        person,
        planComboTestData.planLinkScenario
      );
  ...
  await test.step('Assert alternative medication savings', async () => {
    await claimAlertsMedicationTagsValidator(planComboTestData.tagsStatus);
  });
  ...
  await test.step('Keep current prescription with a link', async () => {
    ...
    await expect(page).toHaveURL(planComboTestData.regExpCabinet);
  });
});
