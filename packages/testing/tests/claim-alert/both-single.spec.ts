...
import { bothSingleTestData, pbmUser } from '../../test-data';
...
test('Keep current prescription @exec', async ({
  ...
  await test.step(
    `Generate Claim Link "${bothSingleTestData.planLinkScenario}" and Navigate to it`,
    async () => {
      const link = await generateClaimLink(
        person,
        bothSingleTestData.planLinkScenario
      );
  ...
  await test.step('Assert alternative medication savings', async () => {
    await claimAlertsMedicationTagsValidator(bothSingleTestData.tagsStatus);
  });
  ...
  await test.step('Keep current prescription with a link', async () => {
    ...
    await expect(page).toHaveURL(bothSingleTestData.regExpCabinet);
  });
});
