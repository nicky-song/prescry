...
const removeDiacritics = (input: string) => {
  defaultDiacriticsRemovalMap.map((currentDiacritic) => {
    input = input.replace(currentDiacritic.letters, currentDiacritic.base);
  });
...
export const inputToASCIIFormatter = (input: string, onlyLetters?: boolean) => {
  const removedDiacriticsInput = removeDiacritics(input);

  return onlyLetters
    ? removeNonLetters(removedDiacriticsInput)
    : removedDiacriticsInput;
};
