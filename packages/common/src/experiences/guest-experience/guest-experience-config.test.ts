...
const expectedContentManagementApi: IApiConfig = {
  env: {
    host: '127.0.0.1',
    port: '443',
    protocol: 'https',
...
const expectedDomainDataApi: IApiConfig = {
  env: {
    host: '127.0.0.1',
    port: '443',
    protocol: 'https',
  ...
  paths: {
    ...
    drugSearch:
      '/dds/2.0/drugs?filter=:filter&groupplancode=:rxSubGroup&maxResults=:maxResults',
    elasticDrugSearch:
      '/drugsearch/3.0/drugs?filter=:filter&groupplancode=:rxSubGroup&maxResults=:maxResults',
    elasticDrugSearchAll:
      '/drugsearch/3.0/drugs?filter=:filter&maxResults=:maxResults',
  },
};

const expectedGuestExperienceApi: IApiConfig = {
  env: {
    host: '127.0.0.1',
    port: '4300',
    protocol: 'https',
...
const mockMemberTerms = ['<MISSING>'];
const mockActionTypeList = [
  'SELECT_OFFER',
  'ACCEPT_OFFER',
...
describe('GuestExperienceConfig', () => {
  it('exports GuestExperienceConfig with defined keys', () => {
    const date = new Date(2019, 1, 1);
    GuestExperienceConfig.currentDate = date;

    const expectedGuestExperienceConfig: IGuestExperienceConfig = {
      allowedActionTypeList: mockActionTypeList,
      apis: {
        contentManagementApi: expectedContentManagementApi,
        domainDataApi: expectedDomainDataApi,
        guestExperienceApi: expectedGuestExperienceApi,
      },
      auth: {
        clientId: '<MISSING>',
        tenantId: '<MISSING>',
    ...
    expect(GuestExperienceConfig).toEqual(expectedGuestExperienceConfig);
  });
});
