...
import { updateRecoveryEmail } from './api-v1.update-recovery-email';
...
const updateRecoveryEmailRequestBody = {
  email: 'test@test.com',
  oldEmail: 'old@test.com',
} as IUpdateRecoveryEmailRequestBody;
...
describe('update recovery email', () => {
  beforeEach(() => {
    mockCall.mockReset();
  ...
  it('makes expected api request', async () => {
    ...
    const expectedUrl = `${protocol}://${host}:${port}${url}${mockConfig.paths.updateRecoveryEmail}`;
    const expectedBody = updateRecoveryEmailRequestBody;
