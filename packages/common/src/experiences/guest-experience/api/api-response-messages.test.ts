...
import {
  AddPinValidationCodes,
  ErrorConstants,
  LoginMessages,
  MemberUpdateRequestValidationCodes,
  SendOneTimePasswordRequestValidationCodes,
  SuccessConstants,
  TwilioErrorMessage,
  UpdatePinValidationMessage,
  VerifyPhoneNumberValidationCodes,
} from './api-response-messages';

describe('LoginMessages', () => {
  it('has expected constants', () => {
    const expectedLoginMessages = {
    ...
    expect(LoginMessages).toEqual(expectedLoginMessages);
  });
});

describe('MemberUpdateRequestValidationCodes', () => {
  it('has expected values', () => {
    const expectedMemberUpdateRequestValidationCodes = {
      EMAIL_ID_REQUIRED: {
        error_code: 'V-110',
    ...
    expect(MemberUpdateRequestValidationCodes).toEqual(
      expectedMemberUpdateRequestValidationCodes
    );
  });
});

describe('TwilioErrorMessage', () => {
  it('has expected constants', () => {
    const expectedTwilioErrorMessage = {
      TOO_MANY_TIMES: 'You have tried too many times',
      UNABLE_TO_SEND_CODE:
        'Unable to send verification code to the provided Number',
      UNSUPPORTED_LANDLINE_NUMBER:
        'SMS is not supported by landline phone number',
      INVALID_EMAIL_FORMAT:
        'We are unable to send you an email. Please try your phone number.',
      INVALID_PHONE_NUMBER: 'Invalid phone number',
    };
    expect(TwilioErrorMessage).toEqual(expectedTwilioErrorMessage);
  });
});

describe('ErrorConstants', () => {
  it('has expected constants', () => {
    const expectedErrorConstants = {
      ...
      UPDATE_EMAIL_ERROR:
        'Old recovery email provided does not match our records',
      ...
      ALTERNATIVE_DRUG_SEARCH_FAILURE:
        'Invalid or missing alternative drug prices',
      ...
      INVALID_ISO_DATE: expect.any(Function),
      INVALID_MEMBER_RXID: 'Invalid primaryMemberRxId',
      ...
      PIN_RESET_LOCKED:
        'Your account is locked due to multiple invalid attempts.',
      PIN_RESET_FAILED:
      ...
      PATIENT_MASTER_ID_MISSING: expect.any(Function),
      PRESCRIPTION_ID_MISSING: 'prescription id is required in the request',
      PRESCRIPTION_MASTER_ID_MISSING:
        'The prescription does not have master id',
      ...
      PRESCRIPTION_UPDATE_FAILURE:
        'Unable to update prescription with memberID',
      ...
      ACCOUNT_CREATION_AGE_REQUIREMENT_NOT_MET: expect.any(Function),
      ...
      INVALID_PRESCRIPTION_DATA:
        'Prescription is missing some required details.',
      ...
      NO_MAIN_OFFER_FOUND: expect.any(Function),
      NO_CONFIRMATION_FOUND: expect.any(Function),
      NO_FILL_OPTIONS_FOUND: expect.any(Function),
      ...
      ACTIVATION_PATIENT_COVERAGES_NOT_FOUND: expect.any(Function),
      ACTIVATION_PATIENT_PHONE_NUMBER_NOT_FOUND: expect.any(Function),
      ACTIVATION_PATIENT_MULTIPLE_PBM_PLANS: expect.any(Function),
      APPOINTMENT_DEPENDENT_NOT_FOUND: expect.any(Function),
      SMART_CONTRACT_ADDRESS_MISSING: 'Smart contract address missing',
    };

    expect(ErrorConstants).toEqual(expectedErrorConstants);

    const dateStringMock = '01-01-01';
    expect(ErrorConstants.INVALID_ISO_DATE(dateStringMock)).toEqual(
      `'${dateStringMock}' is not a valid ISO date.`
    );
    expect(ErrorConstants.INVALID_ISO_DATE(undefined)).toEqual(
      `'' is not a valid ISO date.`
    );

    const phoneNumberMock = 'phone-number';
    expect(ErrorConstants.PATIENT_MASTER_ID_MISSING(phoneNumberMock)).toEqual(
      `Master id missing for patient with phone number ${phoneNumberMock}.`
    );

    const ageMock = 20;
    expect(
      ErrorConstants.ACCOUNT_CREATION_AGE_REQUIREMENT_NOT_MET(ageMock)
    ).toEqual(
      `We currently do not support creating account for under ${ageMock}`
    );

    const identifierMock = 'identifier';
    const referenceNumberMock = 'reference-number';
    expect(ErrorConstants.NO_MAIN_OFFER_FOUND(identifierMock)).toEqual(
      `Main offer not found in offers for identifier: ${identifierMock}`
    );
    expect(ErrorConstants.NO_CONFIRMATION_FOUND(identifierMock)).toEqual(
      `Confirmation not found in prescription for identifier: ${identifierMock}`
    );
    expect(
      ErrorConstants.NO_FILL_OPTIONS_FOUND(identifierMock, referenceNumberMock)
    ).toEqual(
      `No fill options for prescription ${identifierMock}-${referenceNumberMock}`
    );
  });
});

describe('SuccessConstants', () => {
  it('has expected constants', () => {
    const expectedSuccessConstants = {
      ...
      MEMBER_UPDATE_SENT_SUCCESSFULLY:
        'Member update has been sent successfully',
    ...
    expect(SuccessConstants).toEqual(expectedSuccessConstants);
  });
});

describe('SendOneTimePasswordRequestValidationCodes', () => {
  it('has expected values', () => {
    const expectedSendOneTimePasswordRequestValidationCodes = {
      PHONE_NUMBER_REQUIRED: {
        error_code: 'V-2001',
        message: 'Phone number is Required',
      },
    ...
    expect(SendOneTimePasswordRequestValidationCodes).toEqual(
      expectedSendOneTimePasswordRequestValidationCodes
    );
  });
});

describe('VerifyPhoneNumberValidationCodes', () => {
  it('has expected values', () => {
    const expectedVerifyPhoneNumberValidationCodes = {
      CODE_REQUIRED: {
        error_code: 'V-2002',
    ...
    expect(VerifyPhoneNumberValidationCodes).toEqual(
      expectedVerifyPhoneNumberValidationCodes
    );
  });
});

describe('AddPinValidationCodes', () => {
  it('has expected values', () => {
    const expectedAddPinValidationCodes = {
      ENCRYPTED_PIN_REQUIRED: {
        error_code: 'V-2006',
        message: 'Encrypted pin is Required',
      },
    };
    expect(AddPinValidationCodes).toEqual(expectedAddPinValidationCodes);
  });
});

describe('UpdatePinValidationMessage', () => {
  it('has expected values', () => {
    const expectedUpdatePinValidationMessage = {
    ...
    expect(UpdatePinValidationMessage).toEqual(
      expectedUpdatePinValidationMessage
    );
  });
});
