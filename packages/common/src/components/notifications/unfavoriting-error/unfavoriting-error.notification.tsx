...
import { unfavoritingErrorNotificationStyles } from './unfavoriting-error.notification.styles';

export interface IUnfavoritingErrorNotificationProps {
  onClose: () => void;
...
export const UnfavoritingErrorNotification = ({
  onClose,
  viewStyle,
  notificationViewStyle,
}: IUnfavoritingErrorNotificationProps): ReactElement => {
  const groupKey = CmsGroupKey.global;
  const { content, isContentLoading } = useContent<IGlobalContent>(groupKey, 2);
  const message = content.unfavoritingPharmacyError;
  ...
  const baseNotification = (
    <BaseNotification
      ...
      iconSize={iconSize}
      viewStyle={[
        unfavoritingErrorNotificationStyles.notificationViewStyle,
        notificationViewStyle,
      ]}
