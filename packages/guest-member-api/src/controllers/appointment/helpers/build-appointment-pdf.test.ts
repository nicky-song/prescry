...
import { receiptPdfMock } from '../../../mock-data/receipt-pdf-mock.mock';
import { buildAppointmentPdf } from './build-appointment-pdf';
...
describe('buildAppointmentPdf', () => {
  ...
  it('returns patient receipt pdf with appointment data', async () => {
    const appointmentItemMock: IAppointmentItem = {
      ...
      contractFee: 150,
      providerPhoneNumber: '12345',
      providerClia: '123456',
      appointmentLink: 'appointmentLink',
    };
    const appointmentPdf = await buildAppointmentPdf(appointmentItemMock);
    expect(appointmentPdf).toContain(receiptPdfMock.testKitReceipt);
  });

  it('should not render the fields that could be undefined/empty', async () => {
    const appointmentItemMock: IAppointmentItem = {
      serviceName: 'mock-name',
      customerName: 'name',
      customerDateOfBirth: '01/01/2000',
      ...
      providerPhoneNumber: '12345',
      providerClia: undefined,
      appointmentLink: 'appointmentLink',
    };
    const appointmentPdf = await buildAppointmentPdf(appointmentItemMock);
    expect(appointmentPdf).toContain(receiptPdfMock.missingDataReceipt);
  });
});
