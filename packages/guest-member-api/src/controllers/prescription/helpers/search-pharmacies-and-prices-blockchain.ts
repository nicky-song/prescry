...
export async function searchPharmaciesAndPricesBlockchain(
  response: Response,
  prescription: IFhir,
  latitude: number,
  longitude: number,
  ...
  const refillNumber = !refillRequest?.valueString
    ? '1'
    : String(numberOfRepeats - refills);
  const quantity = medicationRequest?.dispenseRequest?.quantity?.value ?? 0;
  const daysSupply =
