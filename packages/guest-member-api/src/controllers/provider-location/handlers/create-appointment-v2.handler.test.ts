...
import * as CreateCheckoutHelper from './create-booking-checkout-session-info';
...
import { getDependentByPatientDetails } from '../../../utils/get-dependent-by-person-details';
...
import { createAppointmentHandlerV2 } from './create-appointment-v2.handler';
import {
  mockChildDependentPatient,
  mockPatient,
} from '../../../mock-data/fhir-patient.mock';
import { getPatientDependentByMasterIdAndRxGroupType } from '../../../utils/fhir-patient/patient.helper';
...
import { buildPatientDetails } from '../helpers/build-patient-details';
import {
  IPatient,
...
jest.mock('../helpers/build-patient-details');
jest.mock('../helpers/update-person-details-if-necessary');
jest.mock('../helpers/update-patient-details-if-necessary');
jest.mock('../../../utils/health-record-event/get-sessionid-from-request');
jest.mock('@phx/common/src/utils/base-64-helper');
jest.mock('../../../utils/get-dependent-by-person-details');
jest.mock(
...
jest.mock(
  '../../../utils/patient/map-patient-to-person-for-appointment.helper'
);
...
const getPatientDependentByMasterIdAndRxGroupTypeMock =
  getPatientDependentByMasterIdAndRxGroupType as jest.Mock;
...
const mapPatientToPersonForAppointmentMock =
  mapPatientToPersonForAppointment as jest.Mock;
...
const buildPatientDetailsMock = buildPatientDetails as jest.Mock;
const updatePersonDetailsIfNecessaryMock =
  updatePersonDetailsIfNecessary as jest.Mock;
const updatePatientDetailsIfNecessaryMock =
  updatePatientDetailsIfNecessary as jest.Mock;
...
const getDependentByPatientDetailsMock =
  getDependentByPatientDetails as jest.Mock;
...
beforeEach(() => {
  ...
  buildPatientDetailsMock.mockResolvedValue(cashPatientInfo);
  ...
  mapPatientToPersonForAppointmentMock.mockReset();
});

describe('createAppointmentHandlerV2 failure cases', () => {
  test.each([undefined, ''])(
  ...
  it('should fail if create booking returns an errorCode', async () => {
    const birthDateMock = '2000-02-02';

    const patientMock = { ...mockPatient, birthDate: birthDateMock };
    ...
    const patientDependentsMock = [
      {
        ...
        childMembers: {
          activePatients: [
            { ...mockChildDependentPatient, birthDate: birthDateMock },
          ],
        },
      },
    ];
    ...
    getRequiredResponseLocalMock
      ...
      .mockReturnValue('accountIdentifier');

    getResponseLocalMock.mockReturnValueOnce(patientDependentsMock);
    getLoggedInUserProfileForRxGroupTypeMock.mockReturnValueOnce(
      cashPersonInfo
    );
    getLoggedInUserPatientForRxGroupTypeMock.mockReturnValueOnce(patientMock);
    updatePersonDetailsIfNecessaryMock.mockReturnValue(cashPersonInfo);
    updatePatientDetailsIfNecessaryMock.mockReturnValue(patientMock);

    getPatientDependentByMasterIdAndRxGroupTypeMock.mockReturnValue(
      mockChildDependentPatient
    );

    mapPatientToPersonForAppointmentMock.mockReturnValue(cashPersonInfo);
  ...
  it('returns failure response if map patient to person helper methods returns undefined', async () => {
    const birthDateMock = '2000-02-02';
    ...
    getResponseLocalMock.mockReturnValue(patientDependentsMock);

    getPatientDependentByMasterIdAndRxGroupTypeMock.mockReturnValue(
      mockChildDependentPatient
    );

    mapPatientToPersonForAppointmentMock.mockReturnValue(undefined);
    ...
    expect(KnownFailureResponseMock).toHaveBeenCalledWith(
      response,
      HttpStatusCodes.BAD_REQUEST,
      ErrorConstants.APPOINTMENT_DEPENDENT_NOT_FOUND(masterIdMock)
    );
...
describe('createAppointmentHandlerV2 success', () => {
  const birthDateMock = '2000-02-02';
  ...
  it('should call create appointment endpoint helper to create appointment', async () => {
    ...
    getRequiredResponseLocalMock.mockReturnValue(patientMock);
    ...
    expect(updatePersonDetailsIfNecessaryMock).toBeCalledWith(
      request,
      cashPersonInfo
    );
  ...
  it('should update address and gender in person and patient', async () => {
    const birthDateMock = '2000-02-02';

    const patientMock = { ...mockPatient, birthDate: birthDateMock };
    const patientProfileMock = { rxGroupType: 'CASH', primary: patientMock };

    const response = {
      locals: {
        personList: [cashPersonInfo],
        patientProfiles: [patientProfileMock],
        accountIdentifier: 'accountIdentifier',
        account: { dateOfBirth: '2000-02-02' },
      },
    } as unknown as Response;
    ...
    getRequiredResponseLocalMock.mockReturnValueOnce(patientMock);
    getResponseLocalMock.mockReturnValueOnce(patientDependentsMock);
    getLoggedInUserProfileForRxGroupTypeMock.mockReturnValueOnce(
      cashPersonInfo
    );
    ...
    expect(updatePatientDetailsIfNecessaryMock).toBeCalledWith(
      request,
      patientMock,
      configurationMock
    );
...
describe('createAppointmentHandlerV2 failure cases for dependents', () => {
  it('should fail if dependent identifier is not in allowed list of dependents', async () => {
    ...
    getPatientDependentByMasterIdAndRxGroupTypeMock.mockReturnValue(undefined);
    ...
    expect(mapPatientToPersonForAppointmentMock).not.toBeCalled();
  });
  ...
  it('should fail if serviceType is covid vaccine and its existing dependent and not meeting 18 year criteria', async () => {
    ...
    const dependentBirthDate = '2019-02-02';
    ...
    const patientDependentsMock = [
      {
        rxGroupType: 'CASH',
        childMembers: {
          activePatients: [patientDependent],
        },
      },
    ];
...
describe('createAppointmentHandlerV2 success cases for dependents', () => {
  const questions = [] as IServiceQuestion[];
  const service = {
    duration: 15,
    questions,
  ...
  it('should return success if identifier is in allowed list of dependents (endpoint version %p)', async () => {
    const birthDateMock = '2000-02-02';

    const dependentBirthDate = '2000-02-02';
    ...
    const cashPersonInfoWithMasterId = {
      ...cashPersonInfo,
      masterId: masterIdMock,
    };

    const dependentPersonInfo = {
      phoneNumber: '1234567890',
      firstName: 'dep first-name',
      lastName: 'dep lastname',
      effectiveDate: '2000-02-02',
      identifier: '',
      rxSubGroup: 'CASH01',
      rxGroup: '200P32F',
      primaryMemberRxId: 'primaryMemberRxId',
    ...
    const response = {
      locals: {
        personList: [cashPersonInfoWithMasterId],
    ...
    getDependentByPatientDetailsMock.mockReturnValue(dependentPersonInfo);

    getPatientAccountByAccountIdMock.mockReturnValue(
      patientAccountPrimaryWithPatientMock
    );

    setPatientAccountStatusToVerifiedMock.mockResolvedValue({});

    updatePatientAccountMock.mockResolvedValue({});
  ...
  it('should create appointment for new dependent if dependent information provided is valid', async () => {
    ...
    const answersMock = [
      {
        ...
        questionText: 'question-text-mock',
        answer: 'Male',
        required: true,
      },
    ] as IQuestionAnswer[];
    ...
    getDependentByPatientDetailsMock.mockReturnValue(undefined);

    createAccountMock.mockReturnValue(patientAccountPrimaryWithPatientMock);
    ...
    const expectedMasterId = 'patient-id';
    const expectedAccountId = 'account-id1';

    const expectedPhoneNumber = '+11111111111';

    expect(assertHasMasterIdMock).toHaveBeenCalledTimes(2);

    expect(assertHasMasterIdMock).toHaveBeenNthCalledWith(
      1,
      masterIdMock,
      expectedPhoneNumber
    );
    ...
    expectToHaveBeenCalledOnceOnlyWith(
      ...
      coverageResponseMock.subscriberId
    );
    ...
    expect(createAccountMock).toHaveBeenNthCalledWith(
      1,
      configurationMock,
      {
        email: '',
        firstName: 'DEP-NAME1',
        isoDateOfBirth: '2017-09-22',
        lastName: 'DEP-NAME',
