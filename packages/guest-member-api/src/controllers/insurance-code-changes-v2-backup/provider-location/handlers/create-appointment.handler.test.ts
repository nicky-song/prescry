...
import * as CreateCheckoutHelper from '../../../provider-location/handlers/create-booking-checkout-session-info';
...
} from '@phx/common/src/models/insurance-card';
...
jest.mock(
  '../../../provider-location/helpers/create-appointment-endpoint.helper'
);
jest.mock(
  '../../../provider-location/helpers/all-required-questions-answered.helper'
);
...
jest.mock('../../../provider-location/helpers/create-appointment-event.helper');
jest.mock('../../../provider-location/helpers/build-dependent-person-details');
jest.mock('../../../provider-location/helpers/build-person-details');
jest.mock('../../../../utils/health-record-event/get-sessionid-from-request');
jest.mock('../../../provider-location/helpers/is-insurance-eligibile');
...
jest.mock('../../../provider-location/helpers/is-dependent-valid');
...
const isInsuranceEligibleMock = isInsuranceEligible as jest.Mock;
...
beforeEach(() => {
  ...
  isInsuranceEligibleMock.mockReturnValue(true);
...
describe('createAppointmentHandler failure cases', () => {
  ...
  it('should fail if insurance eligibility comes back as false', async () => {
    const answers = [] as IQuestionAnswer[];
    const questions = [] as IServiceQuestion[];
    const service = {
      duration: 15,
    ...
    isInsuranceEligibleMock.mockReturnValue(false);
    ...
    const expectedInsuranceCard: IInsuranceCard = {
      dateOfBirth: '2000-02-02',
      firstName: 'first',
      groupId: '',
      lastName: 'last',
    ...
    expect(isInsuranceEligibleMock).toHaveBeenCalledWith(
      expectedInsuranceCard,
      'provider-name',
      configurationMock
    );
  });
...
describe('createAppointmentHandler success', () => {
  ...
  it('for self appointment insuranceInformation should be populated with personInfo information', async () => {
    const insuranceInformationMock: IInsuranceInformation = {
      insuranceCard: {
        firstName: '',
        lastName: '',
  ...
  it('for self appointment insuranceInformation should be populated with account info when personInfo is undefined', async () => {
    const insuranceInformationMock: IInsuranceInformation = {
      insuranceCard: {
        firstName: '',
        lastName: '',
...
describe('createAppointmentHandler success cases for dependents', () => {
  ...
  it('for dependent appointment insuranceInformation should be populated with dependentInfo information when it has identifier defined', async () => {
    const insuranceInformationMock: IInsuranceInformation = {
      insuranceCard: {
        firstName: '',
        lastName: '',
  ...
  it('for dependent appointment insuranceInformation should be populated with dependentInfo information', async () => {
    const insuranceInformationMock: IInsuranceInformation = {
      insuranceCard: {
        firstName: '',
        lastName: '',
