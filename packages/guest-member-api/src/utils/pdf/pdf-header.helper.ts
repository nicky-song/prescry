...
export interface IPdfServiceHeaderProps {
...
export interface IPdfServiceHeaderContent {
  taxIdLabel: string;
  npiLabel: string;
  cliaLabel: string;
...
export const buildPdfServiceHeader = (
  props: IPdfServiceHeaderProps,
  content: IPdfServiceHeaderContent
): Content => {
  const taxIdField = [
    {
      text: `${content.taxIdLabel}: ${props.providerTaxId}`,
  ...
  const npiField = [
    {
      text: `${content.npiLabel} ${props.npi}`,
      border: [false, false, false, false],
      margin: [0, 0, 0, 0],
      fontSize: 9,
      alignment: 'right',
  ...
  const headerRightContent = [
    [
      {
        text: `${props.providerAddress?.address1}, ${props.providerAddress?.city}, ${props.providerAddress?.state}, ${props.providerAddress?.zip}`,
        border: [false, false, false, false],
        margin: [0, 0, 0, 0],
        fontSize: 9,
        alignment: 'right',
    ...
    [
      {
        text: `${content.cliaLabel}: ${props.providerCliaNumber}`,
        border: [false, false, false, false],
        margin: [0, 0, 0, 0],
        fontSize: 9,
        alignment: 'right',
  ...
  if (props.providerPhoneNumber) {
    const providerPhoneNumber = [
      {
        text: `${content.phoneNumberLabel}: ${formatPhoneNumber(
    ...
    headerRightContent.splice(1, 0, providerPhoneNumber);
  }

  if (props.providerTaxId) {
    headerRightContent.push(taxIdField);
  }

  if (props.npi) {
    headerRightContent.push(npiField);
  }
        ...
        [
          {
            ...
            table: {
              ...
              body: [
                [
                  {
                    text: formatProviderName(props.providerName),
                    border: [false, false, false, false],
                    margin: [0, 0, 0, 0],
                    fontSize: 16,
                    alignment: 'left',
