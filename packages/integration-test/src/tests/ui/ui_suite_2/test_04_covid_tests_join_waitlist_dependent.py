...
def test_joinwaitlist_of_covid_tests_for_dependent(page, covid_test_type):
    """
    Test method to join waitlist of all four covid test for dependent - antigen, medicare, medicaid
    :param page: objects for provided page list
    ...
    # Getting mobile number from login.json data
    mobile_number = login_data['waitlist_mobile_number']
    ...
    test_joinwaitlist_of_covid_tests_for_dependent.test_type = covid_test_type['covid_test_selection']


@pytest.mark.dependency(depends=["test_login"])
def test_update_the_covid_tests_dependent_record_from_waitlist_database(page):
    """
    Test method to update the covid tests for dependent record from waitlist database
    :param page: objects for provided page list
    """
    ...
    query_dict, updated_values, updated_record_dict = database.create_query_to_update_the_waitlist_database(
        ...
        service_type=database_data["select_service_type"][test_joinwaitlist_of_covid_tests_for_dependent.test_type],
    ...
    test_update_the_covid_tests_dependent_record_from_waitlist_database.waitlist_url = (
            config_data["waitlist_url"] + identifier
    )


@pytest.mark.dependency(depends=["test_login"])
def test_book_covid_tests_for_dependent_through_waitlist(page):
    """
    Test method to book all four covid tests for myself using identifier - (antigen, medicare, medicaid)
    :param page: objects for provided page list
    """
    covid_test = copy.deepcopy(testdata['covidtest'])
    # Hit url in browser
    page.covidtest.hit_url_in_browser(
        test_update_the_covid_tests_dependent_record_from_waitlist_database.waitlist_url
    )
    # Schedule an appointment for covid vaccine
    covid_test_date_and_time = page.covidtest.schedule_appointment(
        covid_test_type=test_joinwaitlist_of_covid_tests_for_dependent.test_type,
    ...
    if test_joinwaitlist_of_covid_tests_for_dependent.test_type == "antigen":
